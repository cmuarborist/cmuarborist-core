...
from projection_learning_model.constants import SAVE_EVERY
from projection_learning_model.utils import compute_mrr, compute_recall
...
if __name__ == "__main__":
    ...
    num_projections = saved_model['W'].size()[1]
    ...
    # compute projections and loss
    projections = nte.forward(test_embeddings)

    # compute cosine distances and MRR/MAP
    dot_products = -1e12 * torch.ones(len(test_queries),
				       nte.embedding_h.weight.data.size()[0])  # V x N
    # dot_products = torch.mm(projections.squeeze(),
    #                         nte.embedding_h.weight.t())
    internal_dot_products = torch.mm(projections.squeeze(),
				     nte.embedding_h.weight[torch.LongTensor(internal_nodes)].t())
    dot_products[:, torch.LongTensor(internal_nodes)] = internal_dot_products
